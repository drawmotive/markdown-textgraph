...
Release `0.2.2-alpha.1` uses the published TextGraph SDK `0.2.2-alpha.1`, pinned in the independent lockfile.
...
Install `@drawmotive/markdown-it-textgraph@0.2.2-alpha.1` and `markdown-it`.
...
SDK `0.2.2-alpha.1` returns logical display dimensions even when `maxWidth` caps raster resolution. SDK `0.2.0` lacks that metadata: the adapter divides pixel dimensions by the requested scale, so a clamped image also displays smaller with that older SDK. DPI metadata does not control HTML image size.
